...
- **Deterministic seeds in E2E tests:** When E2E tests depend on procedurally-generated graph topology (e.g., star map node adjacency for non-adjacent-node rejection tests), always pin `Math.random()` via `page.addInitScript` in a `beforeEach` hook so the RNG seed is reproducible. The component-level tests already use a fixed seed (`test-seed`); E2E tests must follow the same pattern. Without a fixed seed, the random extra-edges added by `generateStarMap` can create adjacency assumptions that fail ~15% of the time (flaky CI). Verify the chosen seed produces the expected graph topology (e.g., `sys_5` NOT adjacent to `sys_0` or `sys_1`) before committing.
